...
const int row_delta[POSSIBLE_MOVES] = { 2,1,-1,-2,-2,-1,1,2 };
const int col_delta[POSSIBLE_MOVES] = {-1,-2,-2,-1,1,2,2,1 };
...
bool find_tour( int current_row, int current_col,int move_no = 1)
{
	// for debug uncomment two following lines
	// cout << move_no<< endl	;
	// print_board();
	...
	for (int move = 0; move < POSSIBLE_MOVES; move++)
	{
		...
		if (find_tour( new_row, new_col, move_no + 1))
			return true;
...
{
	// board is global var and may have elements
	for (int row = 0; row < ROW_COUNT; row++)
		for (int col = 0; col < COL_COUNT; col++)
			board[row][col] = 0;
	board[init_row][init_col] = 1;
	if (find_tour(init_row, init_col))
		print_board();
	else
		cout << "Failed to find a tour!" << endl;
}
int main()
{
	solve(2, 3);
